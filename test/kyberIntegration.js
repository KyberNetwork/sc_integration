    ...
    describe(test_name, async () => {
      ...
      it('test has network fee camp without winning option, no brr camp, data changes as expected', async () => {
        ...
        await submitNewCampaign(
          ...
          0,
          0,
          [0, defaultNetworkFee - 1, defaultNetworkFee + 1],
          link,
          {from: daoOperator}
        );
      ...
      it('test has network fee camp without winning option, has brr camp without winning option, data changes as expected', async () => {
        ...
        // create brr camp
        await updateCurrentBlockAndTimestamp();
        let newBrrData1 = getDataFromRebateAndReward(
          curBrrData.rebateBps.add(new BN(1)),
          curBrrData.rewardBps.add(new BN(1))
        );
        let newBrrData2 = getDataFromRebateAndReward(
          curBrrData.rebateBps.add(new BN(2)),
          curBrrData.rewardBps.add(new BN(2))
        );
        let newBrrData3 = getDataFromRebateAndReward(
          curBrrData.rebateBps.add(new BN(3)),
          curBrrData.rewardBps.add(new BN(3))
        );
        await submitNewCampaign(
          kyberDao,
          2,
          ...
          0,
          0,
          [newBrrData1, newBrrData2, newBrrData3],
          link,
          {from: daoOperator}
        );
        await Helper.mineNewBlockAt(blockToTimestamp(currentBlock + 2));
...
async function tradeAndCheckDataChangesAsExpected (
  epoch,
  expectedNetworkFee,
  expectedReward,
  expectedRebate,
